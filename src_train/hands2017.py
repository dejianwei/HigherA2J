...
keypointsUVD_test = scio.loadmat(test_keypoint_file)['keypoints3D'].astype(np.float32) # result of A2J
...
def dataPreprocess(index, img, keypointsUVD, center, mean, std, lefttop_pixel, rightbottom_pixel, xy_thres=100, depth_thres=150, augment=False):
    ...
    # print(index, int(new_Ymin), int(new_Ymax), int(new_Xmin), int(new_Xmax), imCrop.shape)
...
def errorCompute(result, target, center):
    ...
    result[:,:,2] = result[:,:,2] / depth_pixel_ratio
